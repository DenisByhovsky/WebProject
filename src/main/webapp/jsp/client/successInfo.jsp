                                ...
                                <div class="infoDialog">
                                    ...
                                    <div class="status_value">
                                        <c:if test="${transaction.transactionType == 'DEPOSIT'}">
                                            <h1><fmt:message key="bl.success"/>
                                            </h1>
                                            <label><fmt:message key="bl.system"/>
                                            </label><span>${transaction.depositType}</span><br>
                                            <label>Deposit:</label>
                                            <span id="win-money">
                <fmt:formatNumber value="${transaction.amount}"/> ${account.invoice.currency}
            </span><br>
                                        </c:if>
                                        <c:if test="${transaction.transactionType == 'DRAWBACK'}">
                                            <h1><fmt:message key="bl.success"/>
                                            </h1>
                                            <label><fmt:message key="bl.system"/>
                                                :</label><span>${transaction.depositType}</span><br>
                                            <label>Drawback:</label> <span id="win-money"><fmt:formatNumber value="${transaction.amount}"/> ${account.invoice.currency}</span><br>
                                        </c:if>
                                        <div class="invoice-status" id="success">Status:
                                            <span><fmt:formatNumber value="${account.invoice.ballance}"/> ${account.invoice.currency}</span>
                                        </div>
                                        <a href="${pageContext.request.contextPath}/jsp/client/cash.jsp">
                ...
                <div class="card">
                    <div class="body bg-cyan">
                        ...
                        <ul class="dashboard-stat-list">
                            <hr>
                            <li>
                                T<c:set var="page" value="${pageContext.request.contextPath}/jsp/client/balanceClient.jsp" scope="session"/>
                                <div class="balance-info">
                                    <div class="invoice-status"><fmt:message key="bl.status"/>
                                        :
                                        <span><fmt:formatNumber value="${account.invoice.ballance}"/> ${account.invoice.currency}</span>
                                    </div>
                                    <form role="form" method="post" action="/controller">
                                        <input type="hidden" name="command" value="invoice_info"/>
                                        <select class="invoice-info-select" name="info_type">
                                            <option value="all">all</option>
                                            <option value="deposit">DEPOSIT</option>
                                            <option value="withdraw">DRAWBACK</option>
                                        </select>

                                        <input type="submit" class="btn" id="invoice-info-btn" value="SHOW">
                                    </form>
                                </div>
                            </li>
